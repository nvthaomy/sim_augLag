...
def CreateOMMSimulation(Sys,system,top,Prefix="",chkfile='checkpnt.chk',Verbose=False):
    ...
    if useNPT:
        ...
        if sum([anisotropicX,anisotropicY,anisotropicZ]) > 0:
            system.addForce(openmm.MonteCarloAnisotropicBarostat(3*[pressure],temperature,anisotropicX,anisotropicY,anisotropicZ,barostatInterval))
            print("Added Anisotropic MC Barostat [x,y,z] = {} with P {} (eps/sig^3), T {}, freq {}".format([anisotropicX,anisotropicY,anisotropicZ],
...
def RunOpenMM(Sys, Prefix = "", Verbose = False, Checkpnt = None, NStepsMin = 0, NStepsEquil = 10000, NStepsProd = 10000, WriteFreq = 10000, TrajFile = "trj.dcd"):
...
#    bondlist = np.array([ [b[0].index,b[1].index] for b in top.bonds() ], dtype=np.intp)
    if len(bondlist) > 0:
    ...
    if NStepsMin > 0 and Checkpnt == None:
    #if False:
...
def MakeOpenMMTraj(Sys, DelTempFiles = False, Prefix = "", Verbose = False, Checkpnt = None, *args, **kwargs):
    """Runs OpenMM and converts into LAMMPS trajectory object that sim can read."""    
    import sim.traj
    import os
    
    OpenMMFiles, DCDFile, returncode = RunOpenMM(Sys, *args, Prefix = Prefix, Verbose = Verbose, Checkpnt = Checkpnt, **kwargs)
    ...
    Lx, Ly, Lz = Sys.BoxL
    BoxL = [Lx, Ly, Lz]
    TrajFile = DCDtoLAMMPS(DCDFile,initialpdb)
...
#    print(data)
    np.savetxt(LogFile, np.array(data), header = """\"Step"\t"Potential Energy (kJ/mole)"\t"Box Volume (nm^3)\" """,comments='#')   
    # --- Returns ---
    return LogFile
