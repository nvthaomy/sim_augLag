...
PairTable2::PairTable2(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
}
...
void PairTable2::compute(int eflag, int vflag)
{
  int i,j,ii,jj,inum,jnum,itype,jtype,itable;
  double xtmp,ytmp,ztmp,delx,dely,delz,evdwl,fpair;
  double rsq,rinv,r,factor_lj,fraction,value,a,b;
  char estr[128];
  int *ilist,*jlist,*numneigh,**firstneigh;
  Table *tb;

  union_int_float_t r_lookup;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        r = sqrt(rsq);
        rinv = 1.0 / MAX(r, 1.e-300);
        tb = &tables[tabindex[itype][jtype]];
        if (r < tb->inner) {
          sprintf(estr,"Pair distance < table inner cutoff: " 
                  "ijtype %d %d dist %g",itype,jtype,r);
        ...
        if (tabstyle == LOOKUP) {
          ...
          fpair = factor_lj * rinv * tb->f[itable];
          ...
          r_lookup.f = r;
          itable = r_lookup.i & tb->nmask;
          itable >>= tb->nshiftbits;
          fraction = (r_lookup.f - tb->r[itable]) * tb->dr[itable];
          value = tb->f[itable] + fraction*tb->df[itable];
          fpair = factor_lj * rinv * value;
        }
...
void PairTable2::settings(int narg, char **arg)
{
...
void PairTable2::coeff(int narg, char **arg)
{
...
double PairTable2::init_one(int i, int j)
{
...
void PairTable2::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
  ...
  for (int i = 0; i < tb->ninput; i++) {
    ...
    if (tb->rflag == RLINEAR)
      ...
      r_lookup.i = i << nshiftbits;
      r_lookup.i |= masklo;
      if (r_lookup.f < tb->rlo) {
        r_lookup.i = i << nshiftbits;
        r_lookup.i |= maskhi;
      }
      rnew = r_lookup.f;
...
void PairTable2::bcast_table(Table *tb)
{
...
void PairTable2::spline_table(Table *tb)
{
...
void PairTable2::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairTable2::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
  ...
  if (tabstyle == SPLINE) {
    ...
    // h(r) = f(r) and g(r) = r
    // dh/dg = df/dr
    // dh/dg in secant approx = (f(r2) - f(r1)) / (r2 - r1)    

    double fp0,fpn;
    double secant_factor = 0.1;
    if (tb->fpflag) fp0 = tb->fplo;
    else {
      double r1 = tb->inner;
      double r2 = r1 + secant_factor*tb->delta;
      fp0 = (splint(tb->rfile,tb->ffile,tb->f2file,tb->ninput,r2) - tb->f[0]) / 
             (secant_factor*tb->delta);
    }

    if (tb->fpflag && tb->cut == tb->rfile[tb->ninput-1]) fpn = tb->fphi;
    else {
      double r2 = tb->cut;
      double r1 = r2 - secant_factor*tb->delta;
      fpn = (tb->f[tlm1] - splint(tb->rfile,tb->ffile,tb->f2file,tb->ninput,r1)) /
             (secant_factor*tb->delta);
    }
  ...
  if (tabstyle == BITMAP) {
    ...
    memory->create(tb->r,ntable,"pair:r");
    ...
    memory->create(tb->dr,ntable,"pair:dr");

    union_int_float_t minr_lookup;
    minr_lookup.i = 0 << tb->nshiftbits;
    minr_lookup.i |= maskhi;

    for (int i = 0; i < ntable; i++) {
      r_lookup.i = i << tb->nshiftbits;
      r_lookup.i |= masklo;
      if (r_lookup.f < tb->inner) {
        r_lookup.i = i << tb->nshiftbits;
        r_lookup.i |= maskhi;
      }
      r = r_lookup.f;
      ...
      minr_lookup.f = MIN(minr_lookup.f,r_lookup.f);
    }

    tb->inner = minr_lookup.f;

    for (int i = 0; i < ntablem1; i++) {
      tb->de[i] = tb->e[i+1] - tb->e[i];
      tb->df[i] = tb->f[i+1] - tb->f[i];
      tb->dr[i] = 1.0/(tb->r[i+1] - tb->r[i]);
    }
    ...
    tb->dr[ntablem1] = 1.0/(tb->r[0] - tb->r[ntablem1]);
...
void PairTable2::null_table(Table *tb)
{
  tb->rfile = tb->efile = tb->ffile = NULL;
  tb->e2file = tb->f2file = NULL;
  tb->r = tb->dr = tb->e = tb->de = NULL;
...
void PairTable2::free_table(Table *tb)
{
...
void PairTable2::spline(double *x, double *y, int n,
...
double PairTable2::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairTable2::write_restart(FILE *fp)
{
...
void PairTable2::read_restart(FILE *fp)
{
...
void PairTable2::write_restart_settings(FILE *fp)
{
...
void PairTable2::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairTable2::single(int i, int j, int itype, int jtype, double rsq,
                         double factor_coul, double factor_lj,
                         double &fforce)
{
  int itable;
  double r,rinv,fraction,value,a,b,phi;
  int tlm1 = tablength - 1;
  ...
  if (r < tb->inner) error->one(FLERR,"Pair distance < table inner cutoff");
  
  if (tabstyle == LOOKUP) {
    itable = static_cast<int> ((r-tb->inner) * tb->invdelta);
    if (itable >= tlm1) error->one(FLERR,"Pair distance > table outer cutoff");
    fforce = factor_lj * rinv * tb->f[itable];
  } else if (tabstyle == LINEAR) {
    itable = static_cast<int> ((r-tb->inner) * tb->invdelta);
    if (itable >= tlm1) error->one(FLERR,"Pair distance > table outer cutoff");
    fraction = (r - tb->r[itable]) * tb->invdelta;
    value = tb->f[itable] + fraction*tb->df[itable];
    fforce = factor_lj * rinv * value;
  } else if (tabstyle == SPLINE) {
    itable = static_cast<int> ((r-tb->inner) * tb->invdelta);
    if (itable >= tlm1) error->one(FLERR,"Pair distance > table outer cutoff");
    b = (r - tb->r[itable]) * tb->invdelta;
...
void *PairTable2::extract(const char *str, int &dim)
{
