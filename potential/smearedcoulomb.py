#/usr/bin/env python
#Added 2019.03.15 by Kevin Shen
...
class SmearedCoulomb(base.PotentialClass):
    """Smeared Coulombic interactions."""

    Names = ["smearedcoulomb"]
...
if (CALCDWPARAM) then
    ...
    if (AIDI==AIDJ) then
        DW_Charge(AIDI) = 2.d0 * Chargej * Coef(0) * (val6 - val2)
        DDW_Charge_BornA(AIDI,0) = 2.d0 * Chargej * Coef(0) * val4
        DDW_Charge_Coef(AIDI,0) = 2.d0 * Chargej * (val6 - val2)
        DDW_Charge_Charge(AIDI,AIDJ) = 2.d0 * Coef(0) * (val6 - val2)
    else
        DW_Charge(AIDI) = Coef(0) * Chargej * (val6 - val2)
        DW_Charge(AIDJ) = Coef(0) * Chargei * (val6 - val2)
        
        DDW_Charge_BornA(AIDI,0) = Coef(0) * val4 * Chargej 
        DDW_Charge_BornA(AIDJ,0) = Coef(0) * val4 * Chargei 
        DDW_Charge_Coef(AIDI,0) = Chargej * (val6 - val2)
        DDW_Charge_Coef(AIDJ,0) = Chargei * (val6 - val2)
        DDW_Charge_Charge(AIDI,AIDJ) = Coef(0) * (val6-val2)
    endif
endif
...
"""
    ...
    def __init__(self, Sys, Label = "", Cut = None, Filter = None,
                 ...
                 BornA = None, Coef = None, Shift = False):
        """Initializes a smeared Coulomb potential."""
        base.PotentialClass.__init__(self, Sys, Label = Label, Cut = Cut,
                                     Filter = Filter,
                                     NType = chem.NPairID(Sys.World.NAID),
                                     Source = SmearedCoulomb.Source)
    ...
    def Val(self, x):
        ...
        v = self.CurrentChargeCoef * erf(np.sqrt(np.pi)*x/2/self.BornA[0]) / x + self.CurrentChargeCoef*self.UShift[0]
    ...
    def DVal(self, x):
        ...
        return self.CurrentChargeCoef * ( np.exp(-fac**2 * x**2) / (x*self.BornA[0]) - erf(fac*x)/x**2  )
