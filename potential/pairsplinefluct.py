...
class PairSplineFluct(base.SplineFluctPotentialClass):
    """Pairwise spline interactions with fluctuations."""

    Names = ["pairsplinefluct"]    
...
"""
    ...
    TestArgs = {"Cut":2.5, "NKnot":5}    
    

    def __init__(self, Sys, Label = "", Cut = None, Filter = None,
                 Fixed = False, Knots = None, NKnot = 20,
                 NonbondEneSlope = "50kTperA", BondEneSlope = "50kTperA",
                 FLNKnot = None):
        """Initializes a B-spline pair potential."""
        if FLNKnot is None: FLNKnot = NKnot
        base.SplineFluctPotentialClass.__init__(self, Sys, Label = Label, Cut = Cut,
                                                Filter = Filter, Fixed = Fixed,
                                                Knots = Knots, NKnot = NKnot,
                                                InnerBC = 2, OuterBC = 3, 
                                                FLInnerBC = 1, FLOuterBC = 3, FLNKnot = FLNKnot,                                        
                                                Source = PairSplineFluct.Source)                                  
        ...
        if self.Filter.Bonded: 
            ...
            self.EneSlopeInner = NonbondEneSlope
