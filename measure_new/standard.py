...
class FluctLnP(base.MeasureAvgClass):
    def __init__(self, Sys, StepFreq = 0, CycleFreq = 1, Active = False):
...
Val = FluctLnP
[DOMEASURE]"""
        base.MeasureAvgClass.__init__(self, Sys, "FluctLnP", StepFreq, CycleFreq, Active,
                                      Source = Source)        

class DFParam(base.MeasureAvgClass):
    
    def __init__(self, Sys, StepFreq = 0, CycleFreq = 1, Active = False):
        """Class for measuring DFParam."""
        object.__setattr__(self, "LibVars", {})
        self.StepFreq = StepFreq
        self.CycleFreq = CycleFreq
        self.Active = Active
...
Val = DFParam
[DOMEASURE]"""
        base.MeasureAvgClass.__init__(self, self.Sys, "DFParam", self.StepFreq,
                                      self.CycleFreq, self.Active, NVal = N1,
                                      UseCovar = True, Report = False,
                                      Source = Source)
        base.MeasureAvgClass.PreLoad(self)
...
class DDFParam(base.MeasureAvgClass):
    def __init__(self, Sys, StepFreq = 0, CycleFreq = 1, Active = True):
        """Class for measuring DDFParam."""
        object.__setattr__(self, "LibVars", {})
        self.StepFreq = StepFreq
        self.CycleFreq = CycleFreq
        self.Active = Active
...
Val = DDFParam
[DOMEASURE]"""        
        base.MeasureAvgClass.__init__(self, self.Sys, "DDFParam", self.StepFreq,
                                      self.CycleFreq, self.Active, NVal = N2,
                                      Report = False, Source = Source)
        base.MeasureAvgClass.PreLoad(self)
    ...
    def PostLoad(self):
        ...
        base.MeasureAvgClass.PostLoad(self)
        #copy the function over so we can convert array to matrix
        self.__MakeMatrix = self.Sys.ForceField.Param.DDF.MakeMatrix         
...
class EinsteinSDC(base.MeasureClass):
    ...
    ModuleVars = base.MeasureClass.ModuleVars + ["PosOrigin", "Sums", "Counts", 
                                                 "MinStep", "MaxStep", "OriginDeltaStep"]
    ModuleConsts = base.MeasureClass.ModuleConsts + ["NOrigins"]
...
def Add(Measures):
    ...
    MList.append(FluctLnP(Sys, StepFreq = 0, CycleFreq = 1, Active = False))  
    MList.append(DFParam(Sys, StepFreq = 0, CycleFreq = 1, Active = False))
    MList.append(DDFParam(Sys, StepFreq = 0, CycleFreq = 1, Active = False))
